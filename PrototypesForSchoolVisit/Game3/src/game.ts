...
export default class Game {
  ...
  private ship_Sally = new Ship({ x: 50, y: 50 }, "Schip Zeelandia", 50);
  private ship_Corkus = new Ship({ x: 350, y: 50 }, "Schip Corkus", 35);
  ...
  public tick() {
    ...
    this.ctx.fillText("Druk op '1' om te verkopen", 700, 200);
    this.ctx.fillText("Druk op '2' om te kopen", 700, 230);
    this.ctx.fillText("Druk op 'spatiebalk' om te reizen", 700, 260);
    this.ctx.lineWidth = 2; this.ctx.font = "bold 20px Navigator"; this.ctx.fillStyle = "brown";
    this.ctx.fillText("Druk op 'x' om te selecteren: Schip Corkus", 700, 160);
    this.ctx.fillText("Druk op 'z' om te selecteren: Schip Zeelandia", 700, 130);

    this.ctx.lineWidth = 2; this.ctx.font = "bold 25px Navigator"; this.ctx.fillStyle = "gold";
    this.ctx.fillText("Schatkist: " + (this.ship_Sally.gold + this.ship_Corkus.gold), 700, 350);

    this.ctx.lineWidth = 2; this.ctx.font = "italic 18px Navigator"; this.ctx.fillStyle = "wheat";
    this.ctx.fillText("Naval Trading Sim Prototype voor het verbeteren van wiskunde door Mecit", 920, 600);
  }
