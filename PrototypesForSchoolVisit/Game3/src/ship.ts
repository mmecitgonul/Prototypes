...
export default class Ship {
  ...
  private cargos: Cargo[] = [new Cargo("Fish", 1, 2, 3), new Cargo("Marble", 4, 20, 5), new Cargo("Cheese", 2, 6, 3), new Cargo("Rifle", 4, 9, 12), new Cargo("Gunpowder", 16, 4, 35)];
  private store: Cargo[] = [new Cargo("Paper", 3, 14, 32), new Cargo("Wheat", 4, 2, 14)];
    ...
    if(this.travelling) {
      ...
      if(this.distance <= 0) {
        ...
        if(this.store.length === 0) {
          this.store.push(new Cargo("Goes cheese", 2, 100, 3));
          this.store.push(new Cargo("Gunpowder", 2, 40, 3));
        }
        else if(this.store.length === 1) {
          this.store.push(new Cargo("jewelry", 2, 20, 3));
        }
      }
    }
  }
  ...
  public draw(ctx: CanvasRenderingContext2D) {
    ...
    ctx.fillText("Encumbrance: " + this.encumbrance, this.pos.x + 20, this.pos.y + 180);
    ctx.lineWidth = 2; ctx.font = "bold 18px Navigator"; ctx.fillStyle = "#FFB200";
    ctx.fillText("Gold: " + this.gold, this.pos.x + 20, this.pos.y + 160);
