...
export default class Game {
  ...
  public update(): void {
    ...
    document.addEventListener('keydown', (e: KeyboardEvent) => {
      if(e.key === '5') {
        ...
        this.ctx.fillText(`Juist!!!`, 250, 550, 1000);

        this.ctx.fillStyle = 'white';
        this.ctx.fillText("Wroom!!", 670, 290, 70);
      }
      else {
        this.ctx.fillStyle = 'red';
        this.ctx.font = '60px Arial';
        this.ctx.fillText(`Fout!!!`, 250, 550, 1000);
      }
    });
  } 
  ...
  private draw(): void {
    ...
    this.ctx.fillText(`snelheid: ${this.panel.speed}m/s`, 500, 50, 1000);
    this.ctx.fillStyle = 'yellow';
    this.ctx.fillText(`Hoogte: ${this.panel.height}m`, 500, 80, 1000);
    this.ctx.fillStyle = 'green';
    this.ctx.fillText(`Afstand: ${this.panel.distance}m`, 500, 110, 1000);

    this.ctx.fillStyle = 'white';
    this.ctx.fillText(`20 - 15 =`, 250, 350, 1000);

    this.ctx.fillStyle = 'brown';
    this.ctx.fillText(`'druk op de juiste cijfertoets'`, 350, 350, 1000);

    this.ctx.fillStyle = 'White';
    this.ctx.font = 'italic 15px Arial';
    this.ctx.fillText(`prototype papieren vliegtuigspel voor het verbeteren van wiskunde door Mecit`, 950, 590, 1000);
  }
