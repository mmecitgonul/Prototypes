...
export default class Game {
  ...
  public tick() {
    ...
    this.ctx.fillText("if <= 10 FAIL", 50, 550);
    this.ctx.fillText("if => 120 FAIL", 50, 580);

    this.ctx.fillStyle = "azure";
    this.ctx.font = "20px sans";
    this.ctx.fillText("Larger = Slower, Stronger Jump", 50, 500);
    this.ctx.fillText("Smaller = Faster, Weaker Jump", 50, 520);

    this.ctx.font = "italic 15px Arial";
    this.ctx.fillText("Puzzle platformer prototype for improving math by Mecit", 960, 600);
